...
> **Router note:** Domain-specific detail lives in per-directory `AGENTS.md`
> files that Hermes auto-loads only when you work in that directory (see
> `agent/subdirectory_hints.py`). This keeps the always-injected root guide
> lean. Pointers below mark what moved where.
...
├── plugins/              # Plugin system (see plugins/AGENTS.md)
...
→ Moved to **`ui-tui/AGENTS.md`** (process model, JSON-RPC transport, Ink
surfaces, slash-command flow, dev commands, dashboard PTY embedding).
Auto-loads when you work under `ui-tui/` or `tui_gateway/`.
...
→ Moved to **`hermes_cli/AGENTS.md`** (skin engine architecture, what skins
customize, built-in skins, adding built-in and user YAML skins).
Auto-loads when you work under `hermes_cli/`.

---

## Plugins

→ Moved to **`plugins/AGENTS.md`** (general plugin surface, memory-provider
plugins, model-provider plugins, context-engine/image-gen directories, and
the "plugins must not modify core" / "no new in-tree memory providers"
policies). Auto-loads when you work under `plugins/`.

---

## Skills

→ Moved to **`skills/AGENTS.md`** (`skills/` vs `optional-skills/`, SKILL.md
frontmatter, and the HARDLINE skill authoring standards). Auto-loads when you
work under `skills/`.
...
→ Moved to **`tests/AGENTS.md`** (why `scripts/run_tests.sh` over bare
`pytest`, subprocess-per-test isolation, local-vs-CI drift table, and the
"don't write change-detector tests" rule). Auto-loads when you work under
`tests/`.
